...
cursor_target.execute(f"""UPDATE  V2I_GFR_Apaleo SET GFR_market_segment= 'MICEGR' WHERE GFR_rateplan LIKE '%GR%' AND GFR_market_segment = 'INDIVIDUAL' AND DATE(v.GFR_datumimp) = '{import_date_str}' """,)
cursor_target.execute(f"""UPDATE  V2I_GFR_Apaleo SET GFR_market_segment= 'LEISUREGR' WHERE GFR_rateplan LIKE '%CR%' AND GFR_market_segment = 'INDIVIDUAL' AND DATE(v.GFR_datumimp) = '{import_date_str}' """,)
