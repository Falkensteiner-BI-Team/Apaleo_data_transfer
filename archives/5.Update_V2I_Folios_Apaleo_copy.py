...
date_ranges_august = [("2024-08-01", "2024-08-05"), ("2024-08-05", "2024-08-10"), ("2024-08-10", "2024-08-15"),
                    ("2024-08-15", "2024-08-20"), ("2024-08-20", "2024-08-25"),
                    ("2024-08-25", "2024-08-30"),("2024-08-30", "2024-09-01")]
...
def Insert_Inhouse_Res(start,end):
    get_reservations = APIClient(
        'https://api.apaleo.com/booking/v1/reservations?dateFilter=Modification&from='  + start + 'T00:00:00Z&to=' + end + 'T00:00:00Z&status=Inhouse',
        get_token()).get_data()
...
for date in date_ranges_august:
    print(date)
...
def Insert_CheckedOut_Res(start,end):
    get_reservations = APIClient(
        'https://api.apaleo.com/booking/v1/reservations?dateFilter=Modification&from=' + start + 'T00:00:00Z&to=' + end + 'T00:00:00Z&status=CheckedOut',
        get_token()).get_data()
...
def Insert_Canceled_NoShow_Res():
    confirmed_services = APIClient(
        'https://api.apaleo.com/booking/v1/reservations/?status=Canceled,NoShow&propertyIds=FKP,FCZ,FHS,FSA,FBL,FST&expand=timeSlices,services',get_token()).get_data()
